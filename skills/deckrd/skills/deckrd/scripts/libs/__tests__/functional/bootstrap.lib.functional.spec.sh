...
Describe "bootstrap.lib.sh"
  ...
  Describe "PROJECT_ROOT: 事前設定維持"
    ...
    It "[Normal] T-LIB-BPRF-02: export 済みなので readonly になっている"
    ...
    It "[Edge] T-LIB-BPRF-03: スペースを含むパスが維持される"
  ...
  Describe "DECKRD_ROOT: BASH_SOURCE 基点"
    ...
    It "[Normal] T-LIB-BROOTF-02: DECKRD_ROOT を固定すると PROJECT_ROOT 変更に影響されない"
    ...
    It "[Edge] T-LIB-BROOTF-03: PROJECT_ROOT=/tmp/bogus を設定しても DECKRD_ROOT は BASH_SOURCE 基点のまま"
  ...
  Describe "DECKRD_SCRIPTS_DIR / DECKRD_LIB_DIR: DECKRD_ROOT 連鎖"
    ...
    It "[Normal] T-LIB-BSCRF-02: DECKRD_LIB_DIR が DECKRD_ROOT/scripts/libs との関係式を満たす"
    ...
    It "[Normal] T-LIB-BSCRF-03: DECKRD_ROOT 変更 → DECKRD_SCRIPTS_DIR が追従する"
    ...
    It "[Edge] T-LIB-BSCRF-04: DECKRD_ROOT を固定しても DECKRD_SCRIPTS_DIR は DECKRD_ROOT に従う"
  ...
  Describe "DECKRD_LOCAL_DATA / DECKRD_DOCS_DIR: PROJECT_ROOT 連鎖"
    ...
    It "[Normal] T-LIB-BLOCDF-02: DECKRD_DOCS_DIR が PROJECT_ROOT/docs/.deckrd との関係式を満たす"
    ...
    It "[Normal] T-LIB-BLOCDF-03: DECKRD_LOCAL_DATA は DECKRD_ROOT に依らず PROJECT_ROOT に依存する"
    ...
    It "[Normal] T-LIB-BLOCDF-04: DECKRD_DOCS_DIR は DECKRD_ROOT に依らず PROJECT_ROOT に依存する"
  ...
  Describe "副作用: 内部変数"
    ...
    It "[Normal] T-LIB-BINTF-02: _BOOTSTRAP_LOADED が設定される"
  ...
  Describe "副作用: シェル環境変数"
    ...
    It "[Normal] T-LIB-BENVF-02: IFS が変化しない"
      When run bash -c "before=\"\${IFS}\"; . \"$SCRIPT\"; [[ \"\${IFS}\" == \"\$before\" ]] && echo ok"
      The status should equal 0
      The output should equal "ok"
    End

    It "[Normal] T-LIB-BENVF-03: PWD が変化しない"
      When run bash -c "before=\"\$PWD\"; . \"$SCRIPT\"; [[ \"\$PWD\" == \"\$before\" ]] && echo ok"
      The status should equal 0
      The output should equal "ok"
    End

    It "[Edge] T-LIB-BENVF-04: OLDPWD が変化しない"
