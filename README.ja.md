...
| **bdd-coder**     | `/bdd-coder:bdd-coder <task-id>` | BDD スタイルでの実装（実装フェーズ）               |
...
フェーズ 3: 実装・リリース（bdd-coder + IDD）
  /bdd-coder:bdd-coder T01 → BDD 実装 → /idd-commit-message → /idd-pr → PR
```

## インストール

### Agent Skills として利用する場合（Claude Code、Codex など）
...
### Claude Code の plugin として利用する場合
...
# フェーズ 3: 実装とリリース（bdd-coder + IDD）
/bdd-coder:bdd-coder T01-01    # タスク T01-01 を実装
/bdd-coder:bdd-coder T01-02    # タスク T01-02 を実装
# ... (タスクごとに実行)
/idd-commit-message
git commit -m "..."
/idd-pr
...
| `status`                        | ワークフロー進捗を確認                           |
| `review <phase>`                | ドキュメントをレビュー（任意のフェーズで使用可） |
...
`bdd-coder` は、deckrd 利用者向けに提供される **オプションのスキル**です。
deckrd で `tasks.md` を生成した後、自動実装が必要な場合に使用します。

**完全にオプション**: deckrd だけでタスクを生成し、自分のペースで手動実装することもできます。

### インストール

`bdd-coder` は deckrd マーケットプレイスに含まれており、deckrd と一緒にインストールできます。

```bash
# gh skills（deckrd と bdd-coder を両方インストール）
gh skills install aglabo/deckrd

# または claude plugin で個別にインストール
claude plugin install bdd-coder@deckrd
```
