...
1. Read CC value
2. Read branch/line coverage % — if unavailable, use `coverage = 0` (worst-case fallback)
3. Compute CRAP
4. Classify: ≤15 PASS / 16–30 WARN / >30 CRITICAL

**Coverage unavailable fallback**: substitute `coverage = 0` → `CRAP = CC × (CC + 1)`.
Any function with CC ≥ 6 becomes CRITICAL under this fallback.
Mark the score as `cov=N/A` in the report.
