...
deckrd-coder requires an existing Deckrd workflow with the following files generated:

- `tasks/tasks.md` — task definitions (generated by `/deckrd tasks`)
- `tasks/implementation-checklist.md` — task checklist (generated by `/deckrd tasks`)
- deckrd-coder operates exclusively on tasks defined in these files
...
The implementation follows a 7-phase workflow (Phase 0–6):
...
Phase 1: Deckrd Session & Task Information Extraction
    ↓
Phase 2: Dependency Map (Serial / Parallel Task Classification)
    ↓
...
Phase 5: Completion Verification
    ↓
Phase 6: Session End (Reset state; remind user to commit manually)
```

For detailed phase descriptions, see `skills/deckrd-coder/references/workflow.md`.
...
Comprehensive documentation is included in `skills/deckrd-coder/references/`:

- `SKILL.md`: Command reference and feature overview
- `references/workflow.md`: 7-phase workflow details
- `references/implementation.md`: Implementation overview
- `references/troubleshooting.md`: Error handling and recovery
- `references/faq.md`: Common questions and answers
- `agents/bdd-coder.md`: Internal BDD agent specification
