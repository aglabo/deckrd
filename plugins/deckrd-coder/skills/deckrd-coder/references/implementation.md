...
### ステップ 2: チェックリストファイルから実装タスクリストを取得する

目的: チェックリストファイルから対象タスクの Red-Green-Refactor サイクルを把握する。

#### チェックリストファイルの解決

以下の優先順位でチェックリストファイルを決定する。

1. `--checklist <path>` オプションが指定された場合 → そのパスを使用
2. オプション未指定の場合 → `tasks/implementation-checklist.md` を使用

#### 実行内容

1. チェックリストファイルを読み込む
2. 指定されたタスク ID (例: `T01-02`) に対応するセクションを抽出
3. 該当セクションの Red (`-R`) / Green (`-G`) / Refactor (`-F`) チェック項目を確認
4. 未完了 (`[ ]`) の項目を実装対象として把握

#### チェックリスト項目例

```markdown
#### T-01-02-01 — `"  Alice  "` → `"Hello, Alice!"`

- [ ] **[T-01-02-01-R] Red** : `Greet("  Alice  ")` のテストを書き、失敗を確認する
- [ ] **[T-01-02-01-G] Green** : テストが通る最小限の実装を行う
- [ ] **[T-01-02-01-F] Refactor** : 実装を整理し、テストが引き続き通ることを確認する
```

チェックリスト:

- [ ] チェックリストファイルが存在し、読み込めた
- [ ] 対象タスク ID のセクションが存在する
- [ ] R/G/F 各フェーズのチェック項目を把握した
...
状態確定とは、タスク完了の事実を確認し (実装済みチェック項目を更新)、TodoWrite で同期し、次タスクに進んでよい状態であることを明示するフェーズです。

実行内容:

1. チェックリストファイル (`implementation-checklist.md` またはオプション指定ファイル) を読み込む
2. 完了した各フェーズのチェック項目 (`-R` / `-G` / `-F`) を `[x]` に更新
3. TodoWrite で進捗を同期

更新例:

```markdown
- [x] **[T-01-02-01-R] Red** : `Greet("  Alice  ")` のテストを書き、失敗を確認する
- [x] **[T-01-02-01-G] Green** : テストが通る最小限の実装を行う
- [x] **[T-01-02-01-F] Refactor** : 実装を整理し、テストが引き続き通ることを確認する
```

チェックリスト:

- [ ] 完了フェーズのチェック項目 (R/G/F) がすべて `[x]` 更新済み
- [ ] TodoWrite に反映済み
